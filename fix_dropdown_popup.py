...
target = """                              <MoreVertical className="w-4 h-4" />
                            </button>"""

replacement = """                              <MoreVertical className="w-4 h-4" />
                            </button>
                            {/* Dropdown Popup */}
                            {activeDropdownCustomerId === customer?.id && (
                              <div className="absolute right-0 mt-8 w-64 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 dark:border-slate-800 z-50 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 animate-in fade-in zoom-in-95 duration-100">
...
# Replace ONLY the first occurrence which is in the main layout (the one we added).
